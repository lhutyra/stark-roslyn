...
namespace StarkPlatform.CodeAnalysis.Editor.CSharp.Formatting
{
    [ExportLanguageService(typeof(IEditorFormattingService), LanguageNames.Stark), Shared]
    internal partial class CSharpEditorFormattingService : IEditorFormattingService
    {
        ...
        public bool SupportsFormattingOnTypedCharacter(Document document, char ch)
        {
            ...
            var smartIndentOn = options.GetOption(FormattingOptions.SmartIndent, LanguageNames.Stark) == FormattingOptions.IndentStyle.Smart;
            ...
            var autoFormattingOnTyping = options.GetOption(FeatureOnOffOptions.AutoFormattingOnTyping, LanguageNames.Stark);
            if (!autoFormattingOnTyping)
            {
                return false;
            }

            if (ch == '}' && !options.GetOption(FeatureOnOffOptions.AutoFormattingOnCloseBrace, LanguageNames.Stark))
            {
                return false;
            }

            if (ch == ';' && !options.GetOption(FeatureOnOffOptions.AutoFormattingOnSemicolon, LanguageNames.Stark))
            {
                return false;
            }
