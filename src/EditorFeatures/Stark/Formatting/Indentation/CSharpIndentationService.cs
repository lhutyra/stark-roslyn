...
using StarkPlatform.CodeAnalysis.Editor.Implementation.SmartIndent;
...
{
    [ExportLanguageService(typeof(ISynchronousIndentationService), LanguageNames.Stark), Shared]
    internal partial class CSharpIndentationService : AbstractIndentationService<CompilationUnitSyntax>
    {
            ...
            CancellationToken cancellationToken)
        {
            ...
            if (!optionSet.GetOption(FeatureOnOffOptions.AutoFormattingOnReturn, LanguageNames.Stark))
            {
                return false;
            }

            if (optionSet.GetOption(FormattingOptions.SmartIndent, LanguageNames.Stark) != FormattingOptions.IndentStyle.Smart)
            {
                return false;
            }
