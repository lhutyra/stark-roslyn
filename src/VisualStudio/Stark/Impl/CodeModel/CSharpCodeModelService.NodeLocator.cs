        ...
        private class NodeLocator : AbstractNodeLocator
        {
            ...
            protected override VirtualTreePoint? GetStartPoint(SourceText text, OptionSet options, SyntaxNode node, EnvDTE.vsCMPart part)
            {
                switch (node.Kind())
                {
                    ...
                    case SyntaxKind.ImportDirective:
                        return GetStartPoint(text, (ImportDirectiveSyntax)node, part);
            ...
            protected override VirtualTreePoint? GetEndPoint(SourceText text, OptionSet options, SyntaxNode node, EnvDTE.vsCMPart part)
            {
                switch (node.Kind())
                {
                    ...
                    case SyntaxKind.ImportDirective:
                        return GetEndPoint(text, (ImportDirectiveSyntax)node, part);
                    case SyntaxKind.EnumMemberDeclaration:
                ...
                switch (part)
                {
                    ...
                    case EnvDTE.vsCMPart.vsCMPartNavigate:
                        ...
                        if (firstAccessorNode != null)
                        {
                            ...
                            else if (!firstAccessorNode.EosToken.IsMissing)
                            {
                                // This is total weirdness from the old C# code model with auto props.
                                // If there isn't a body, the semi-colon is used
                                return GetBodyStartPoint(text, options, firstAccessorNode.EosToken, firstAccessorNode.EosToken, indentation);
                            }
                        }
            ...
            private VirtualTreePoint GetStartPoint(SourceText text, ImportDirectiveSyntax node, EnvDTE.vsCMPart part)
            {
                int startPosition;
                    ...
                    case EnvDTE.vsCMPart.vsCMPartNavigate:
                        ...
                        if (firstAccessorNode != null)
                        {
                            ...
                            else
                            {
                                // This is total weirdness from the old C# code model with auto props.
                                // If there isn't a body, the semi-colon is used
                                return GetBodyEndPoint(text, firstAccessorNode.EosToken);
                            }
                        }
            ...
            private VirtualTreePoint GetEndPoint(SourceText text, ImportDirectiveSyntax node, EnvDTE.vsCMPart part)
            {
                int endPosition;
