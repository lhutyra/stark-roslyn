...
using StarkPlatform.VisualStudio.LanguageServices.CSharp.CodeModel.Extenders;
using StarkPlatform.VisualStudio.LanguageServices.CSharp.CodeModel.MethodXml;
...
namespace StarkPlatform.VisualStudio.LanguageServices.CSharp.CodeModel
{
    internal partial class CSharpCodeModelService : AbstractCodeModelService
    {
            ...
            switch (node.Kind())
            {
                ...
                case SyntaxKind.ImportDirective:
                    if (scope == EnvDTE.vsCMElement.vsCMElementImportStmt &&
                        ((ImportDirectiveSyntax)node).Name != null)
                    {
                        return true;
                    }
        ...
        private static IEnumerable<SyntaxNode> GetAttributeNodes(SyntaxList<AttributeSyntax> attributeDeclarationList)
        {
            foreach (var attribute in attributeDeclarationList)
            {
            ...
            foreach (var member in GetChildMemberNodes(container))
            {
                if (member is BaseFieldDeclarationSyntax baseField)
                {
                    ...
                    if (logicalFields)
                    {
                        ...
                        yield return baseField.Declaration;
                    }
            ...
            SyntaxNode node)
        {
            ...
            switch (node.Kind())
            {
                ...
                case SyntaxKind.ImportDirective:
                    return CreateInternalCodeImport(state, fileCodeModel, (ImportDirectiveSyntax)node);
            }
        ...
        {
            return node is ImportDirectiveSyntax;
        }
        ...
        {
            if (node.Kind() == SyntaxKind.ImportDirective)
            {
                throw Exceptions.ThrowEFail();
            }
        ...
        {
            foreach (ImportDirectiveSyntax usingDirective in GetImportNodes(parentNode))
            {
        ...
        {
            Debug.Assert(attributeNode is AttributeSyntax);
            Debug.Assert(attributeNode.Parent is AttributeSyntax);

            return (AttributeSyntax)attributeNode.Parent;
        }
        ...
        public override string GetAttributeTarget(SyntaxNode attributeNode)
        {
            ...
            var attributeList = (AttributeSyntax)attributeNode.Parent;
        ...
        public override SyntaxNode SetAttributeTarget(SyntaxNode attributeNode, string target)
        {
            ...
            var attributeList = (AttributeSyntax)attributeNode;
        ...
        public override SyntaxNode GetEffectiveParentForAttribute(SyntaxNode node)
        {
            ...
            {
                return node.GetAncestor<BaseFieldDeclarationSyntax>().Declaration;
            }
        ...
        public override SyntaxNode CreateAttributeNode(string name, string value, string target = null)
        {
            ...
            return SyntaxFactory.Attribute(specifier, 
        ...
        public override SyntaxNode CreateImportNode(string name, string alias = null)
        {
            ...
            if (!string.IsNullOrEmpty(alias))
            {
                ...
                return SyntaxFactory.ImportDirective(aliasSyntax, nameSyntax);
            }
            else
            {
                return SyntaxFactory.ImportDirective(nameSyntax);
            }
        }
        ...
        {
            if (importNode is ImportDirectiveSyntax usingDirective)
            {
        ...
        public override SyntaxNode SetMustImplement(SyntaxNode memberNode, bool value)
        {
            ...
            if (value)
            {
                ...
                if (member is MethodDeclarationSyntax method)
                {
                    ...
                    {
                        member = method.WithBody(null).WithEosToken(SyntaxFactory.Token(SyntaxTriviaList.Create(SyntaxFactory.ElasticMarker), SyntaxKind.SemicolonToken, method.Body.CloseBraceToken.TrailingTrivia));
                    }
                }
                else
                {
                    ...
                    if (member is BasePropertyDeclarationSyntax property && property.AccessorList != null)
                    {
                        ...
                        foreach (var accessor in property.AccessorList.Accessors)
                        {
                            ...
                            var updatedAccessor = accessor.WithBody(null).WithEosToken(SyntaxFactory.Token(SyntaxTriviaList.Create(SyntaxFactory.ElasticMarker), SyntaxKind.SemicolonToken, accessor.Body.CloseBraceToken.TrailingTrivia));
            ...
            else
            {
                ...
                if (member is MethodDeclarationSyntax method)
                {
                    if (method.Body == null)
                    {
                        ...
                        newBody = newBody.WithCloseBraceToken(newBody.CloseBraceToken.WithTrailingTrivia(method.EosToken.TrailingTrivia));
                        member = method.WithEosToken(default).WithBody(newBody);
                    }
                }
                else
                {
                    ...
                    if (member is BasePropertyDeclarationSyntax property && property.AccessorList != null)
                    {
                        ...
                        foreach (var accessor in property.AccessorList.Accessors)
                        {
                            ...
                            var newBody = SyntaxFactory.Block();
                            newBody = newBody.WithCloseBraceToken(newBody.CloseBraceToken.WithTrailingTrivia(accessor.EosToken.TrailingTrivia));
                            var updatedAccessor = accessor.WithEosToken(default).WithBody(newBody);
                            updatedAccessors.Add(updatedAccessor);
                        }
        ...
        protected override int GetAttributeIndexInContainer(SyntaxNode containerNode, Func<SyntaxNode, bool> predicate)
        {
            ...
            while (index < attributes.Length)
            {
                ...
                {
                    var attributeDeclaration = (AttributeSyntax)attribute.Parent;
        ...
        protected override SyntaxNode InsertAttributeListIntoContainer(int index, SyntaxNode list, SyntaxNode container)
        {
            ...
            if (container is CompilationUnitSyntax compilationUnit)
            {
                var newAttributeLists = compilationUnit.AttributeLists.Insert(index, (AttributeSyntax)list);
                return compilationUnit.WithAttributeLists(newAttributeLists);
            }
            else if (container is EnumDeclarationSyntax enumDeclaration)
            {
                var newAttributeLists = enumDeclaration.AttributeLists.Insert(index, (AttributeSyntax)list);
                return enumDeclaration.WithAttributeLists(newAttributeLists);
            }
            else if (container is ClassDeclarationSyntax classDeclaration)
            {
                var newAttributeLists = classDeclaration.AttributeLists.Insert(index, (AttributeSyntax)list);
            ...
            {
                var newAttributeLists = structDeclaration.AttributeLists.Insert(index, (AttributeSyntax)list);
            ...
            {
                var newAttributeLists = interfaceDeclaration.AttributeLists.Insert(index, (AttributeSyntax)list);
                return interfaceDeclaration.WithAttributeLists(newAttributeLists);
            }
            else if (container is MethodDeclarationSyntax method)
            {
                var newAttributeLists = method.AttributeLists.Insert(index, (AttributeSyntax)list);
                return method.WithAttributeLists(newAttributeLists);
            }
            else if (container is OperatorDeclarationSyntax operationDeclaration)
            {
                var newAttributeLists = operationDeclaration.AttributeLists.Insert(index, (AttributeSyntax)list);
            ...
            {
                var newAttributeLists = delegateDeclaration.AttributeLists.Insert(index, (AttributeSyntax)list);
                return delegateDeclaration.WithAttributeLists(newAttributeLists);
            }
            else if (container is EnumMemberDeclarationSyntax member)
            {
                var newAttributeLists = member.AttributeLists.Insert(index, (AttributeSyntax)list);
                return member.WithAttributeLists(newAttributeLists);
            }
            else if (container is ParameterSyntax parameter)
            {
                var newAttributeLists = parameter.AttributeLists.Insert(index, (AttributeSyntax)list);
        ...
        {
            var import = (ImportDirectiveSyntax)importNode;
