...
namespace StarkPlatform.VisualStudio.LanguageServices.CSharp.Options
{
    ...
    public class AutomationObject
    {
        ...
        public string Style_NamingPreferences
        {
            get
            {
                return _workspace.Options.GetOption(SimplificationOptions.NamingPreferences, LanguageNames.Stark).CreateXElement().ToString();
            }
            ...
            set
            {
                try
                {
                    _workspace.Options = _workspace.Options.WithChangedOption(SimplificationOptions.NamingPreferences, LanguageNames.Stark, NamingStylePreferences.FromXElement(XElement.Parse(value)));
                }
                catch (Exception)
                {
                }
        ...
        {
            return _workspace.Options.GetOption(key, LanguageNames.Stark) ? 1 : 0;
        }

        private T GetOption<T>(PerLanguageOption<T> key)
        {
            return _workspace.Options.GetOption(key, LanguageNames.Stark);
        }
        ...
        {
            _workspace.Options = _workspace.Options.WithChangedOption(key, LanguageNames.Stark, value != 0);
        }

        private void SetOption<T>(PerLanguageOption<T> key, T value)
        {
            _workspace.Options = _workspace.Options.WithChangedOption(key, LanguageNames.Stark, value);
        }

        private int GetBooleanOption(PerLanguageOption<bool?> key)
        {
            var option = _workspace.Options.GetOption(key, LanguageNames.Stark);
            if (!option.HasValue)
            {
                return -1;
            }
        ...
        private void SetBooleanOption(PerLanguageOption<bool?> key, int value)
        {
            ...
            _workspace.Options = _workspace.Options.WithChangedOption(key, LanguageNames.Stark, boolValue);
        }

        private string GetXmlOption(PerLanguageOption<CodeStyleOption<bool>> option)
        {
            return _workspace.Options.GetOption(option, LanguageNames.Stark).ToXElement().ToString();
        }
        ...
        private void SetXmlOption(PerLanguageOption<CodeStyleOption<bool>> option, string value)
        {
            ...
            _workspace.Options = _workspace.Options.WithChangedOption(option, LanguageNames.Stark, convertedValue);
        }
    }
}
